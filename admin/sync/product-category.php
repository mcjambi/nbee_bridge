...
function nbee_sync_product_category()
{
    ...
    while ($is_more_data) {
        ...
        foreach ($categories as $category) {
            nbee_handle_product_category($category);
        }
...
function nbee_sync_single_product_category($category_id)
{
    $nbee_backend_crm_uri = get_option('nbee_backend_crm_uri');
    $nbee_backend_xsigned = get_option('nbee_backend_xsigned');
    ...
    // Gửi request tới API với tham số phân trang
    $response = wp_remote_get(
        $nbee_backend_crm_uri . '/sync/product_category/' . $category_id,
        array(
            'headers' => array(
                'x-signed' => $nbee_backend_xsigned
            ),
    ...
    $category = json_decode(wp_remote_retrieve_body($response), true);
...
function nbee_handle_product_category($category)
{
    ...
    if (isset($category['category_thumbnail_to_media'])) {
